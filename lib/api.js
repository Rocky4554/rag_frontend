...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Profile check: attempt silent refresh, but NEVER redirect (auth-context handles null user)
    if (error.response?.status === 401 && !originalRequest._retry && isProfileCheck) {
    ...
    // Other 401s on non-auth endpoints: attempt refresh, then redirect to login on failure
...
export const uploadAPI = {
  ...
  uploadText: (text, title) =>
    api.post("/api/upload/text", { text, title }),
};
...
export const chatAPI = {
  ...
  /** Returns a ReadableStream of SSE tokens via fetch (not axios) */
  sendStream: (sessionId, question) =>
...
export const interviewAPI = {
  start: (sessionId, maxQuestions = 5) => {
    ...
    return api.post("/api/interview/start", { sessionId, maxQuestions, timezone });
  },
};
...
export const historyAPI = {
  ...
  deleteDocument: (sessionId) => api.delete(`/api/history/documents/${sessionId}`),
...
export const conversationalAiAPI = {
  start: (sessionId) => api.post("/api/conversational-ai/start", { sessionId }),
  stop: (sessionId) => api.post("/api/conversational-ai/stop", { sessionId }),
};
