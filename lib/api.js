...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Allow ONLY /api/auth/profile to trigger auto-refresh. Skip refresh for other auth endpoints (login, signup, logout, refresh)
    const allowRefresh = originalRequest.url?.includes('/api/auth/profile');

    if (error.response?.status === 401 && !originalRequest._retry && allowRefresh) {
      if (isRefreshing) {
